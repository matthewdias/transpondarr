...
export function DownloadSection({ settings }: { settings: Settings }) {
  ...
  const body = (): DownloadInput => ({ url, user, password: password || undefined, category })

  const test = useMutation({
    mutationFn: () => api.testDownload(body()),
    onSuccess: () => setTestState({ ok: true, message: 'Connected to qBittorrent.' }),
    onError: (e) => setTestState({ ok: false, message: e instanceof Error ? e.message : String(e) }),
  })
  const save = useMutation({ mutationFn: () => api.updateDownload(body()), ...useSaveToast(queryClient, 'Download client') })
  ...
  return (
    ...
    >
      <Field label="WebUI URL" placeholder="http://localhost:8080" value={url} onChange={(e) => setUrl(e.target.value)} />
      <div className="grid gap-3.5 sm:grid-cols-2">
        <Field label="Username" value={user} onChange={(e) => setUser(e.target.value)} autoComplete="off" />
        <Field
          label="Password"
          type="password"
          value={password}
          ...
          hint={d.password_set ? 'Leave blank to keep the stored password.' : undefined}
