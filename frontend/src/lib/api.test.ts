...
import { api, ApiError, AUTH_EXPIRED_EVENT, UnauthorizedError } from '@/lib/api'
...
afterEach(() => {
  for (const l of listeners.splice(0)) window.removeEventListener(AUTH_EXPIRED_EVENT, l)
})
...
describe('typed client (openapi-fetch)', () => {
  ...
  it('dispatches the auth-expired event on a stale-session 401', async () => {
    ...
    server.use(http.get('/api/v1/series', () => new HttpResponse(null, { status: 401 })))
...
describe('auth endpoints (rawFetch)', () => {
  ...
  it('keeps a failed login quiet so the form can render its own error', async () => {
    const listener = watchAuthExpired()
    server.use(
      http.post('/api/v1/auth/login', () => new HttpResponse('invalid credentials', { status: 401 })),
    )
    await expect(api.login('example-user', 'wrong')).rejects.toBeInstanceOf(UnauthorizedError)
