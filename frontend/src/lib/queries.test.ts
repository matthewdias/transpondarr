...
describe('query key factories', () => {
  it('produce identical keys for identical arguments', () => {
    expect(seriesDetailQuery(7).queryKey).toEqual(seriesDetailQuery(7).queryKey)
  ...
  it('scope parameterized keys by their argument', () => {
    expect(seriesDetailQuery(1).queryKey).not.toEqual(seriesDetailQuery(2).queryKey)
    expect(metadataSearchQuery('a').queryKey).not.toEqual(metadataSearchQuery('b').queryKey)
