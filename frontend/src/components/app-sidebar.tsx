...
export function AppSidebar() {
  ...
  const system: NavItem[] = [{ title: 'Settings', to: '/settings', icon: Settings }]
    ...
    <Sidebar collapsible="offcanvas">
      <SidebarHeader>
        <div className="flex items-center gap-2.5 px-2 py-2">
          ...
          <div className="text-[15px] font-semibold tracking-tight">Transpondarr</div>
        </div>
      </SidebarHeader>
      ...
      <SidebarFooter>
        ...
        {auth?.session ? (
          ...
          <div className="flex items-center gap-2 px-3 py-2 text-xs text-muted-foreground">
            <ShieldCheck className="size-4" /> Local access — no sign-in required
          </div>
        ) : null}
      </SidebarFooter>
    </Sidebar>
