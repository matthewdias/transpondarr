-- +goose Up
-- What the last pass decided about one wanted item (issue #181). A refusal exists
-- only in memory for the length of a pass, so "the queue got here and declined"
-- was indistinguishable from "nothing has looked at this yet". One outcome row per item,
...
-- outcome row correctly means no pass has written one.
