-- +goose Up
-- One-time data fix on live instances, not a schema change. Grab rows deferred by the
...
-- +goose Down
-- Irreversible: these grab rows are indistinguishable from ones deferred afterwards.
SELECT 1;
