...
// The provider-identity migration rebuilds series to shed the single-column
// UNIQUE on anilist_id, and series has three ON DELETE CASCADE children. A
...
func TestProviderIdentityMigrationKeepsCascadeChildren(t *testing.T) {
	...
	if err := st.DB.QueryRowContext(ctx,
		`INSERT INTO series (anilist_id, title, format, monitored, quality_profile_id, pinned_group, search_backoff)
		 VALUES (4321, 'Populated Show', 'TV', 1, 7, 'ExampleSubs', 3) RETURNING id`).Scan(&seriesID); err != nil {
	...
	for _, seed := range []struct {
		...
		{"grab event", `INSERT INTO grab_events (series_id, wanted_item_id, info_hash, event) VALUES (?, ?, 'abc', 'grabbed')`, []any{seriesID, itemID}},
		{"blocklist entry", `INSERT INTO release_blocklist (series_id, info_hash, release_title, normalized_title, reason) VALUES (?, 'def', '[Other] Show - 01', 'other show 01', 'import failed')`, []any{seriesID}},
	} {
	...
	if err := st.DB.QueryRowContext(ctx,
		...
		seriesID).Scan(&provider, &providerID, &title, &profileID, &pinnedGroup, &searchBackoff); err != nil {
	...
	// The cascade must still be wired after the rebuild, or a deleted series
	// leaves orphans forever.
	if _, err := st.DB.ExecContext(ctx, `DELETE FROM series WHERE id = ?`, seriesID); err != nil {
...
func TestSeriesIdentityIsThePair(t *testing.T) {
	st := tempStore(t)
	ctx := context.Background()
...
// exists to remove.
func TestSeriesIdentityIsBothOrNeither(t *testing.T) {
	st := tempStore(t)
	ctx := context.Background()
