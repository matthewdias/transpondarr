...
- A DB change = a goose migration under `internal/store/migrations` + queries in
  ...
  - **Keep comments in `internal/store/queries/*.sql` ASCII-only.** sqlc's sqlite
    codegen miscounts byte vs. rune offsets. If a doc comment between `-- name:`
    and the SQL body contains a multi-byte character (eg. an em dash, which this
    repo's prose style uses everywhere), it silently truncates the *emitted* SQL
    by that many bytes. The result compiles, `make gen` reports no error, and
    the query fails only at runtime.
  - **`sqlc.arg(name)` is rejected by this dialect** (`extraneous input '?1'`) —
...
- **Why a rebuild is dangerous.** SQLite has no DROP CONSTRAINT, so changing one
  means create-copy-drop-rename. And `DROP TABLE series` with foreign keys on
  **cascade-deletes the user's whole library**, since `db.go` enables them in the
  DSN for every pooled connection.
- **The pragma and the DROP must run on the same connection.** `PRAGMA
  foreign_keys` is a silent no-op inside a transaction (hence `-- +goose NO
  TRANSACTION`) *and* is per-connection. One statement block is one `Exec` is one
  pooled connection — that is the guarantee; loose statements have none.
- **Wrap the DDL in an explicit `BEGIN`/`COMMIT`** so a failure rolls back.
- **Restore `PRAGMA foreign_keys = on` inside the same block**, because the DSN
  pragma is applied only at connection open.
- **The acceptance criterion is a migration test**, not a nicety: it seeds every
  cascade child and asserts each child is still there.
