...
// stagingSuffixes are the staging names a transfer writes beside its destination.
...
// SweepStaging removes staging files no transfer is writing and older than
// olderThan, reporting how many it removed (#132).
func (t *Target) SweepStaging(ctx context.Context, olderThan time.Duration) (int, error) {
	...
	for _, s := range stale {
		...
		if err != nil {
			...
			// enumerated twice, or an import reclaimed its own staging file since the walk.
			if !errors.Is(err, fs.ErrNotExist) {
				t.log.Warn("mediaserver: stale staging file could not be removed", "path", s.path, "err", err)
			}
			continue
		}
		if !gone {
		...
		removed++
		t.log.Info("mediaserver: removed a stale staging file",
...
func (t *Target) collectStale(ctx context.Context, cutoff time.Time) ([]staleStaging, error) {
	var out []staleStaging
	for _, root := range t.stagingRoots() {
		walk := func(path string, d fs.DirEntry, err error) error {
			...
			if err != nil {
				t.log.Debug("mediaserver: staging sweep skipped a path", "path", path, "err", err)
				return nil
			}
			if d.IsDir() || !isStagingName(d.Name()) {
				return nil
			}
			info, err := d.Info()
			...
			out = append(out, staleStaging{path: path, modTime: info.ModTime()})
			return nil
		}
...
// stagingRoots resolves the configured roots, since WalkDir will not descend a
...
func (t *Target) stagingRoots() []string {
	out := make([]string, 0, 2)
...
// isStagingName reports whether name is one of our staging names over a video file —
