...
func TestReplaceOverwritesALargerDestination(t *testing.T) {
	...
	r.Replace = true
	dest, err := New(Roots{Series: root}, LayoutSeasonFolders, "copy", nil).Place(t.Context(), r)
	if err != nil {
...
func TestPlaceWithoutReplaceKeepsTheLargerDestination(t *testing.T) {
	...
	dest, err := New(Roots{Series: root}, LayoutSeasonFolders, "copy", nil).Place(t.Context(), req(src, "Placeholder Saga", 3))
	if err != nil {
		t.Fatalf("Place: %v", err)
	}
...
func TestReplaceInLinkModeSwapsAtomically(t *testing.T) {
	...
	r := req(src, "Placeholder Saga", 3)
	r.Replace = true
	got, err := New(Roots{Series: root}, LayoutSeasonFolders, "hardlink", nil).Place(t.Context(), r)
	if err != nil {
		t.Fatalf("Place: %v", err)
	}
	if got != dest {
