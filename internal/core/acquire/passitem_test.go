...
// A held item is passed to the matcher as a candidate with its release name:
// the case the InLibrary/grabbable split was made for (#97).
func TestPassItemsCarriesHeldIdentityToTheMatcher(t *testing.T) {
	...
	got := matchItems(passItems([]sweepItem{
		...
		{number: 3, inLibrary: true, grabbable: false, heldTitle: held}, // excluded: candidacy is grabbable's job
	}))
...
func TestPassItemsReportsPossessionAndCandidacySeparately(t *testing.T) {
	got := passItems([]sweepItem{
		{number: 1, inLibrary: false, grabbable: false}, // in flight or unaired: not grabbable, not in library
		{number: 2, inLibrary: true, grabbable: false},  // imported: not grabbable, in library
