...
- **Checking the release feed no longer costs more the more titles you track.**
  Every release on the feed was being read once per tracked title rather than
  once, and episodes already in the library were being examined even when the
  quality profile has upgrades switched off. Neither happens now, so a large
  library spends noticeably less time on each check.
